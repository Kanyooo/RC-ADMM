...
Paper-oriented design:
  Experiment 1: finite-budget solver performance on QP-lift/SOCP/SDP,
...
Important choices:
  ...
  - QP is represented as a lifted conic linear program using rotated SOC blocks,
    so the same CLP/ADMM layer is used for QP, SOCP, and SDP.
...
CONFIG: Dict[str, Any] = {
    ...
    "dtype": torch.float32,
    "root_out_dir": "./rc_admm_config_outputs_v2",
    "quick": False,
    ...
    for exp in config["experiments"]:
        ...
        for problem in exp["problems"]:
            for scale in exp["scales"]:
                if config.get("quick", False) and not (problem == "socp" and scale == "small"):
                    continue
                print("\n" + "#" * 100)
